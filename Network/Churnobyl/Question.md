...
## 네트워크의 안정성을 높이기 위한 주요 방법은 무엇인가요?
    이중화/다중화 구성이나 로드 밸런서 설정, 그리고 지속적인 모니터링 등으로 안정성을 높일 수 있습니다.
## 네트워크 장애가 발생했을 때 어떻게 진단하고 해결하나요?
    먼저 ping 명령으로 해당 네트워크 접속이 가능한지 확인하고 traceroute 명령으로 경로에 문제가 없는지 알아봅니다. 그리고 netstat, tcpdump 명령으로 패킷 흐름을 점검해 장애의 원인을 찾고 네트워크 구성 오류나 장비의 문제를 수정합니다.
## 패킷 손실이 발생했을 때 이를 감지하고 복구하는 방법은 무엇인가요?
    Wireshark로 손실률을 측정하고 네트워크 혼잡이나 불량한 장비를 교체합니다.
## 대칭키 암호화와 비대칭키 암호화의 차이를 설명해주세요.
    대칭키는 하나의 키로 암호화/복호화를 수행해 속도가 빠르지만 키의 보안을 유지하기 어렵습니다. 반대로 비대칭키는 공개키와 개인키로 구성돼 보안성을 높이지만 상대적으로 속도가 느립니다.
## HTTPS가 안전한 이유를 설명해주세요.
    HTTPS는 TLS 기반 암호화로 HTTP요청을 암호화해 데이터를 안전하게 전송합니다. CA를 통해 서버가 발급받은 디지털 인증서를 통해 서버의 신뢰성을 검증해 중간자 공격을 방지할 수 있습니다.
## SSL과 TLS의 차이는 무엇인가요?
    TLS는 SSL의 개선된 버전입니다. 보안 취약점이 개선되었고 더 강력한 암호화 알고리즘과 향상된 핸드셰이크 프로토콜을 사용합니다.
## 디지털 인증서란 무엇이며, 어떻게 동작하나요?
    디지털 인증서는 Certificate Authority 즉 CA가 발급하며 인증서를 해싱한 값을 CA의 개인키로 암호화한 서명값과 인증서를 클라이언트에게 보냅니다. 클라이언트는 CA의 공개키로 암호화한 서명값을 다시 해싱값으로 복호화하고 인증서를 해싱한 뒤 두 값을 비교해서 일치하면 CA가 발급한 인증서임을 보장할 수 있습니다.
## PKI(Public Key Infrastructure)의 주요 구성 요소를 설명해주세요.
    PKI는 인증 기관인 CA, 등록 기관인 RA, 인증서 저장소, 키 생성/관리 시스템으로 구성되며 안전한 공개키 기반 인증을 제공합니다.
## MITM(Man-In-The-Middle) 공격이란 무엇이며, 이를 방어하기 위한 방법은 무엇인가요?
    MITM는 공격자가 통신 중간에 개입해 데이터를 가로채거나 변조하는 공격으로 HTTPS, VPN, HSTS, DNSSEC 설정 등으로 방어할 수 있습니다.
## 무선 네트워크의 장단점은 무엇인가요?
    무선 네트워크는 설치와 이동이 자유롭고 확장하기 쉽습니다. 하지만 신호 간섭에 취약하고 유선에 비해 속도나 보안에 취약할 수 있습니다.
## Wi-Fi와 LTE의 차이점을 설명해주세요.
    WI-FI는 AP를 이용한 근거리 통신으로 주로 실내에서 사용되고, LTE는 기지국 기반의 이동통신망으로 광범위한 지역에서 빠른 데이터 전송이 가능합니다.
## WPA2와 WPA3의 차이점은 무엇인가요?
    WPA3는 SAE를 통한 키 교환, 오프라인 공격 방어, 각 연결에 개별 암호화를 적용해 WPA2보다 보안성을 향상시켰습니다.
## 무선 네트워크에서 채널 간섭 문제를 해결하는 방법은 무엇인가요
    5GHz 대역을 사용하거나 채널을 자동으로 선택하는 기능을 활성화해 간섭이 없는 채널을 선택할 수 있습니다.
## 무선 네트워크에서의 QoS(Quality of Service)는 어떻게 구현되나요?
    무선 네트워크에서 대역폭 관리와 트래픽 우선순위 설정을 통해 지연이나 손실을 최소화할 수 있습니다.